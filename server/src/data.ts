...
// In a real application, this would be replaced with a connection to a real database
// like Firebase, PostgreSQL, etc. The functions exported from this file would
// then perform database queries instead of operating on an in-memory object.

let mockData: MockData | null = null;

const initializeMockData = (): MockData => {
    console.log("Initializing Mock Data on the server...");
...
export const getAllAssets = (siteId: string | null = null): Asset[] => {
    const db = getDb();
    let sites = db.enterprise.sites;
    if (siteId) {
        sites = sites.filter(s => s.id === siteId);
    }
    return sites.flatMap(site => site.areas.flatMap(area => area.units.flatMap(unit => unit.assets)));
...
export const getAssetById = (assetId: string): Asset | undefined => {
    return getAllAssets().find(a => a.id === assetId);
}

export const updateAsset = (updatedAsset: Asset): Asset | undefined => {
    const db = getDb();
    let found = false;
    db.enterprise.sites = db.enterprise.sites.map(s => ({
        ...s,
        areas: s.areas.map(a => ({
            ...a,
            units: a.units.map(u => ({
                ...u,
                assets: u.assets.map(as => {
                    if (as.id === updatedAsset.id) {
                        found = true;
                        return updatedAsset;
                    }
                    return as;
    ...
    return found ? updatedAsset : undefined;
}

export const addWorkOrder = (workOrder: WorkOrder): WorkOrder => {
    const db = getDb();
    db.workOrders.push(workOrder);
    return workOrder;
}

export const updateWatchList = (watchList: string[]): string[] => {
    const db = getDb();
